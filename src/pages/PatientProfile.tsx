...
export default function PatientProfile() {
  ...
  const useMutationWithInvalidation = (mutationFn: any, queryKeysToInvalidate: string[]) => useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        ...queryKeysToInvalidate.map((key: string) => queryClient.invalidateQueries({ queryKey: [key, patientId] })),
    ...
    switch (type) {
      ...
      case 'labOrder':
        setModalContent({
          ...
          form: <LabOrderForm labOrder={item} onSubmit={(data) => handleFormSubmit(labOrderMutation, data, item)} {...commonProps} isSubmitting={labOrderMutation.isPending} patientId={patientId} encounterId={null} />
        });
        break;
      case 'telemedicine':
        setModalContent({
  ...
  const timelineEvents = React.useMemo(() => {
    ...
    if (Array.isArray(encounters)) {
      encounters.forEach((enc) => {
        if (enc?.visit_date && enc?.id) {
          events.push({
            id: `enc-${enc.id}`,
            date: enc.visit_date,
            type: 'encounter',
            title: enc.chief_complaint || 'Clinical Visit',
            icon: FileHeart,
            data: enc,
    ...
    if (Array.isArray(prescriptions)) {
      prescriptions.forEach((rx) => {
        if (rx?.start_date && rx?.id) {
          events.push({
            id: `rx-${rx.id}`,
            date: rx.start_date,
            type: 'prescription',
            title: `Prescribed ${rx.medication_name || 'Medication'}`,
            icon: Pill,
            data: rx,
    ...
    if (Array.isArray(labOrders)) {
      labOrders.forEach((lab) => {
        if (lab?.order_date && lab?.id) {
          events.push({
            id: `lab-${lab.id}`,
            date: lab.order_date,
            type: 'lab_order',
            title: `Lab Order: ${lab.test_name || 'Test'}`,
            icon: Beaker,
            data: lab,
            onEdit: () => openModal('labOrder', lab)
          });
        }
      });
    }
  ...
  }, [patient, appointments, encounters, prescriptions, labOrders, openModal]);
  ...
  if (!patient) {
    return (
      <div className="flex flex-col items-center justify-center h-screen text-center p-8">
        ...
        <p className="text-gray-600 mb-4">The patient you're looking for could not be loaded.</p>
        <Button onClick={() => navigate('/patients')}>Return to Patients List</Button>
      </div>
    );
  }
  ...
  const latestVitals = encounters && Array.isArray(encounters) && encounters.length > 0 ?
    encounters
      .filter((e) => e?.vital_signs && Object.keys(e.vital_signs).length > 0)
      .sort((a, b) => new Date(b?.visit_date || 0).getTime() - new Date(a?.visit_date || 0).getTime())[0]?.vital_signs :
    null;
                  ...
                  <Tabs defaultValue="appointments" className="w-full">
                    ...
                    <TabsContent value="appointments" className="relative">
                      ...
                      <PatientAppointments
                        ...
                        onEdit={(apt) => openModal('appointment', apt)}
                        onStatusChange={(apt, status) => appointmentMutation.mutate({ ...apt, status })}
                      />
                    </TabsContent>
                  ...
                  <Tabs defaultValue="procedures" className="w-full">
                    ...
                    <TabsContent value="procedures" className="relative">
                      ...
                      <PatientProcedures
                        ...
                        onEdit={(proc) => openModal('procedure', proc)}
                      />
                    </TabsContent>
                    ...
                    <TabsContent value="surgeries" className="relative">
                      ...
                      <PatientSurgeries
                        surgeries={surgeries || []}
                        isLoading={loadingSurgeries}
                        onEdit={(surgery) => openModal('surgery', surgery)}
                      />
                    </TabsContent>
                    ...
                    <TabsContent value="vaccinations" className="relative">
                      ...
                      <PatientVaccinations
                        ...
                        onEdit={(vax) => openModal('vaccination', vax)}
                      />
                    </TabsContent>
                  </Tabs>
                </TabsContent>
                  ...
                  <Tabs defaultValue="consultations" className="w-full">
                    ...
                    <TabsContent value="referrals" className="relative">
                      ...
                      <PatientReferrals
                        ...
                        onEdit={(ref) => openModal('referral', ref)}
                      />
                    </TabsContent>
                  </Tabs>
                </TabsContent>
                  ...
                  <Tabs defaultValue="consents" className="w-full">
                    ...
                    <TabsContent value="consents" className="relative">
                      ...
                      <PatientConsents
                        ...
                        onEdit={(consent) => openModal('consent', consent)}
                      />
                    </TabsContent>
                    ...
                    <TabsContent value="discharge" className="relative">
                      ...
                      <PatientDischargeSummaries
                        summaries={dischargeSummaries || []}
                        isLoading={loadingDischargeSummaries}
                        onEdit={(summary) => openModal('dischargeSummary', summary)}
                      />
                    </TabsContent>
                    ...
                    <TabsContent value="files" className="relative">
                      ...
                      <PatientDocuments
                        ...
                        onEdit={(doc) => openModal('document', doc)}
                      />
                    </TabsContent>
                  </Tabs>
                </TabsContent>
                ...
                <TabsContent value="billing" className="p-6">
                  <div className="relative">
                    ...
                    <PatientBilling
                      ...
                      onEdit={(bill) => openModal('billing', bill)}
                    />
                  </div>
                </TabsContent>
