...
export default function RecentBillings({ billings, isLoading }: RecentBillingsProps) {
  if (isLoading) {
    return (
          ...
          recentBillings.map((bill: any) => (
            <div key={bill.id} className="flex items-center justify-between p-3 rounded-lg bg-white border border-gray-200 hover:shadow-md transition-shadow">
              ...
              <div className="text-right">
                ...
                <Badge className={`${statusColors[bill.status as keyof typeof statusColors]} text-xs`}>{bill.status.replace('_', ' ')}</Badge>
              </div>
            </div>
          ))
        )}
